...
export function EstimationCardSelector({ enabled, onVote, votedValue, scale = "fibonacci" }: EstimationCardSelectorProps) {
  ...
  return (
      <div className="mb-16 grid grid-cols-4 md:grid-cols-6 lg:grid-cols-9 gap-16 max-w-4xl mx-auto">
        {currentScale.map((value) => (
          <PokerCard key={value} value={value} selected={value == votedValue} onClick={() => onVote(value)} disabled={!enabled} size="md"/>
        ))}
      </div>
  )
}
